...
class player():
    ...
    def check(self):
        # Get the player name.
        art.cd(5, '', "", '', False)
        playername = input("What is your callsign? ")
        art.cd(4, '', "", 0, False)
        # Check the DB for player.
        connection = db.stdb()
        query = "select * from `players` where `callsign` = '"+playername+"'"
        results = db.rowcount(connection, query)
        if results == 0:
            # Register player
            print("")
            art.cd(2,'',"You were not found in the player database.",'',True)
            art.cd(255,4,"Would you like to start a new character in this game? (Type ",'',False)
            art.cd(3,'',"Y",'',False)
            ...
            if command == "y" or command == "Y" or command == "":
                ...
                art.cd('cyan','',"Let's get some paperwork out of the way.",'',True)
                print("")
                art.cd('cyan','',"What is your first name? ",0,True)
                fname = input("")
                art.cd('cyan','',"If you have a middle name, let me know. Press 'Enter' to skip. ",0,True)
                mname = input("")
                art.cd('cyan','',"Last, but not least, what is your last name? ",0,True)
                lname = input("")
                branch = ""
                while branch == "":
                    art.cd('cyan','',"Which branch of service are you most interested in? (",'',False)
                ...
                print("")
                art.cd('cyan','',"Give me a moment to sign you in...Okay, you're all set!",'',True)
                userid="1"
